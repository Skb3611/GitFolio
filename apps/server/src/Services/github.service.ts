...
export const getUserRepos = async ({
  ...
  try {
    const res = token
      ? await fetch(`${BASE_URL}/user/repos`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      : await fetch(`${BASE_URL}/users/${username}/repos`, {
          headers: {
            Authorization: `Bearer ${config.GITHUB_ACCESS_TOKEN!}`,
          },
        });
    const data = await res.json();
    let repos: Promise<Repo[]> = Promise.all(
...
async function makeGraphQLQuery(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `GitHub GraphQL API HTTP error: ${response.status} - ${JSON.stringify(errorData.errors || errorData)}`
      );
    }
...
export const getUserContributionsGraph = async (
  ...
  try {
    ...
    for (let year = firstContributionYear; year <= currentYear; year++) {
      ...
      console.log(
        `Fetching contributions for ${year}... (from: ${fromDate}, to: ${toDate})`
      );

      // Execute the GraphQL query for the current year
      const yearContributionsData = await makeGraphQLQuery(
        token,
        contributionsQuery,
        {
          username: username,
      ...
      if (
        yearContributionsData?.user?.contributionsCollection
          ?.contributionCalendar
      ) {
        allContributions.push({
          year: year,
          data: yearContributionsData.user.contributionsCollection
            .contributionCalendar,
        });
      } else {
        console.warn(
          `No contribution data found for ${year} or unexpected structure.`
        );
      }
    }

    console.log(
      `Successfully fetched contributions for ${allContributions.length} years.`
    );
