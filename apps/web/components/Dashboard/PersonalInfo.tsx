...
const PersonalInfoTab = ({
  ...
  return (
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>Update your personal details and profile information</DialogDescription>
            </DialogHeader>
            <div className="space-y-6 py-4">
              <div className="flex items-center gap-4">
                <Avatar className="h-20 w-20">
                  <AvatarImage src={editInfo.profileImg || "/placeholder.svg"} alt={editInfo.full_name} />
                  <AvatarFallback>
                    {editInfo.full_name
                      .split(" ")
                      .map((n) => n[0])
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setEditInfo({ ...editInfo, tagline: e.target.value })}
    ...
    <CardContent className="space-y-6">
      <div className="flex items-center gap-4">
        <Avatar className="h-20 w-20">
          <AvatarImage src={info.profileImg || "/placeholder.svg"} alt={info.full_name} />
          <AvatarFallback>
            {info.full_name
              .split(" ")
              .map((n) => n[0])
        ...
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">{info.tagline.length==0?"No tagline":info.tagline}</p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          <Input value={`https://github.com/${info.username}`} disabled className="bg-muted" />
        </div>
      </div>

      <div>
        <h4 className="font-medium mb-2">Bio</h4>
        <p className="text-muted-foreground">{info.bio.length==0? "No bio added":info.bio}</p>
      </div>
    </CardContent>
  </Card>
  );
