...
const CTASection = () => {
  ...
  return (
    <div className="relative flex w-full justify-center items-center overflow-hidden py-14">
      <div className="opacity-60 w-full overflow-hidden" >
        <Marquee className="[--duration:20s] mb-1" pauseOnHover={false}>
          {[...firstRow, ...firstRow].map((item, idx) => (
            <ImageItem key={`row1-${idx}`} item={item} idx={idx} />
          ))}
        </Marquee>
        <Marquee reverse className="[--duration:25s] mb-1" pauseOnHover={false}>
          {[...secondRow, ...secondRow].map((item, idx) => (
            <ImageItem key={`row2-${idx}`} item={item} idx={idx} />
          ))}
        </Marquee>
        <Marquee className="[--duration:30s] mb-1" pauseOnHover={false}>
          {[...thirdRow, ...thirdRow].map((item, idx) => (
            <ImageItem key={`row3-${idx}`} item={item} idx={idx} />
          ))}
        </Marquee>
        <Marquee reverse className="[--duration:35s] mb-1" pauseOnHover={false}>
          {[...firstRow, ...firstRow].map((item, idx) => (
            <ImageItem key={`row4-${idx}`} item={item} idx={idx} />
          ))}
        </Marquee>
        <Marquee className="[--duration:40s] mb-1" pauseOnHover={false}>
          {[...secondRow, ...secondRow].map((item, idx) => (
            <ImageItem key={`row5-${idx}`} item={item} idx={idx} />
          ))}
        </Marquee>
        <Marquee reverse className="[--duration:45s]" pauseOnHover={false}>
          {[...thirdRow, ...thirdRow].map((item, idx) => (
            <ImageItem key={`row6-${idx}`} item={item} idx={idx} />
          ))}
        </Marquee>
        
        {/* Gradient overlay */}
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-background to-70% dark:to-background pointer-events-none"></div>
      </div>
      
      <div className="w-full absolute -translate-y-10 flex flex-col justify-center items-center z-10">
        <motion.div
          initial={{
            opacity: 0,
            scale: 0.5,
